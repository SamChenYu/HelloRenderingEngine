...
namespace avocet::opengl {
    ...
    struct vao_lifecycle_events {
        ...
        static void generate(raw_indices<N>& indices) { gl_function{&GladGLContext::GenVertexArrays}(N, indices.data()); }

        template<std::size_t N>
        static void destroy(const raw_indices<N>& indices) { gl_function{&GladGLContext::DeleteVertexArrays}(N, indices.data()); }

        static void bind(const resource_handle& h) { gl_function{&GladGLContext::BindVertexArray}(get_index(h)); }
    ...
    struct common_buffer_lifecycle_events {
        ...
        template<std::size_t N>
        static void generate(raw_indices<N>& indices) { gl_function{&GladGLContext::GenBuffers}(N, indices.data()); }

        template<std::size_t N>
        static void destroy(const raw_indices<N>& indices) { gl_function{&GladGLContext::DeleteBuffers}(N, indices.data()); }
    };
    ...
    struct buffer_lifecycle_events : common_buffer_lifecycle_events {
        ...
        static void bind(const resource_handle& h) { gl_function{&GladGLContext::BindBuffer}(to_gl_enum(Species), get_index(h)); }

        static void configure(const resource_handle& h, const configurator& config) {
            add_label(identifier, h, config.label);
            gl_function{&GladGLContext::BufferData}(to_gl_enum(Species), sizeof(T) * config.buffer_data.size(), config.buffer_data.data(), GL_STATIC_DRAW);
        }
    };
    ...
    private:
        ...
        void set_attribute_ptr(attrib_ptr_info& info, std::size_t sizeofAtt, GLsizei stride) {
            ...
            if constexpr(std::is_same_v<ValueType, GLdouble>) {
                gl_function{&GladGLContext::VertexAttribLPointer}(info.index, components, typeSpecifier, stride, (GLvoid*)info.offset);
            }
            else {
                gl_function{&GladGLContext::VertexAttribPointer}(info.index, components, typeSpecifier, GL_FALSE, stride, (GLvoid*)info.offset);
            }
            gl_function{&GladGLContext::EnableVertexAttribArray}(info.index);
    ...
    public:
        ...
        friend std::vector<T> extract_data(const generic_buffer_object& gbo) {
            ...
            gl_function{&GladGLContext::GetBufferSubData}(to_gl_enum(Species), 0, size, buffer.data());
            return buffer;
        }
    private:
        [[nodiscard]]
        static GLint get_buffer_size() {
            GLint param{};
            gl_function{&GladGLContext::GetBufferParameteriv}(to_gl_enum(Species), GL_BUFFER_SIZE, &param);
            return param;
        }
    };
