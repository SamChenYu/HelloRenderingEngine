...
namespace avocet::opengl {
    ...
    struct texture_2d_lifecycle_events {
        ...
        static void generate(raw_indices<N>& indices) { gl_function{&GladGLContext::GenTextures}(N, indices.data()); }

        template<std::size_t N>
        static void destroy(const raw_indices<N>& indices) { gl_function{&GladGLContext::DeleteTextures}(N, indices.data()); }

        static void bind(const resource_handle& h) { gl_function{&GladGLContext::BindTexture}(GL_TEXTURE_2D, get_index(h)); }
    ...
    public:
        ...
        friend void bind(const texture_2d& tex, texture_unit unit) {
            gl_function{&GladGLContext::ActiveTexture}(unit.gl_texture_unit());
