...
namespace avocet::opengl {
    ...
    struct texture_2d_configurator {
        ...
        std::function<void(const GladGLContext&)> parameter_setter;
    ...
    struct texture_2d_lifecycle_events {
        ...
        static void bind(const GladGLContext& ctx, const resource_handle& h) { gl_function{&GladGLContext::BindTexture}(ctx, GL_TEXTURE_2D, get_index(h)); }
    ...
    public:
        ...
        [[nodiscard]]
        friend unique_image extract_data(const GladGLContext& ctx, const texture_2d& tex2d, texture_format format, alignment rowAlignment) {
            return do_extract_data(ctx, tex2d, format, rowAlignment);
        }

        friend void bind(const GladGLContext& ctx, const texture_2d& tex, texture_unit unit) {
            gl_function{&GladGLContext::ActiveTexture}(ctx, unit.gl_texture_unit());
            base_type::do_bind(ctx, tex);
        }
    private:
        static unique_image do_extract_data(const GladGLContext& ctx, const texture_2d& tex2d, texture_format format, alignment rowAlignment);
    };
}
