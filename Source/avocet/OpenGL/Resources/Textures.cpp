...
namespace avocet::opengl {
    ...
    void texture_2d_lifecycle_events::configure(const GladGLContext& ctx, const resource_handle& h, const configurator& config) {
        add_label(ctx, identifier, h, config.label);
        load_to_gpu(ctx, config);
        if(config.parameter_setter)
            config.parameter_setter();
    }

    [[nodiscard]]
    unique_image texture_2d::do_extract_data(const GladGLContext& ctx, const texture_2d& tex2d, texture_format format, alignment rowAlignment) {
        texture_2d::do_bind(ctx, tex2d);
        ...
        gl_function{&GladGLContext::GetTexImage}(ctx,
            GL_TEXTURE_2D,
            0,
            to_gl_enum(format),
            to_gl_enum(to_gl_type_specifier_v<value_type>),
