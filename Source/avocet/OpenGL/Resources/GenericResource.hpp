...
namespace avocet::opengl {
    ...
    public:
            ...
            : m_Resource{ctx}
        {
            for(const auto& [handle, config] : std::views::zip(get_handles(), configs)) {
                ...
                lifecycle_type::bind(get_context(), handle);
                lifecycle_type::configure(get_context(), handle, config);
            }
        }
        ...
        template<std::size_t I>
            requires (I < N)
        [[nodiscard]]
        std::string extract_label(index<I> i) const { return get_object_label(get_context(), LifeEvents::identifier, get_handle(i)); }
    ...
    protected:
        ...
        template<std::size_t I>
            requires (I < N)
        static void do_bind(const generic_resource& gbo, index<I> i) { lifecycle_type::bind(gbo.get_context(), gbo.get_handle(i)); }

        static void do_bind(const generic_resource& gbo) requires (N == 1) { do_bind(gbo, index<0>{}); }

        [[nodiscard]]
        const GladGLContext& get_context() const noexcept { return m_Resource.get_context(); }
    private:
        [[nodiscard]]
