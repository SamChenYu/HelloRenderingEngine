...
namespace avocet::opengl {
    ...
    namespace {
        ...
        public:
            ...
            [[nodiscard]]
            resource_handle create() { return resource_handle{gl_function{&GladGLContext::CreateShader}(to_gl_enum(m_Species))}; }

            static void destroy(const resource_handle& handle) { gl_function{&GladGLContext::DeleteShader}(get_index(handle)); }
        };
        ...
        public:
                ...
                : m_Resource{species}
            {
                ...
                gl_function{&GladGLContext::ShaderSource}(index, 1, &data, nullptr);
                gl_function{&GladGLContext::CompileShader}(index);
        ...
        public:
            ...
            {
                gl_function{&GladGLContext::AttachShader}(m_ProgIndex, m_ShaderIndex);
            }

            ~shader_attacher() { gl_function{&GladGLContext::DetachShader}(m_ProgIndex, m_ShaderIndex); }
        };
    ...
    shader_program::shader_program(const std::filesystem::path& vertexShaderSource, const std::filesystem::path& fragmentShaderSource) {
...

        {
            ...
            gl_function{&GladGLContext::LinkProgram}(progIndex);
            ...
            if(object_labels_activated()) {
                ...
                gl_function{&GladGLContext::ObjectLabel}(GL_PROGRAM, progIndex, to_gl_sizei(label.size()), label.data());
            }
        }
    ...
    GLint shader_program::extract_uniform_location(std::string_view name) {
        ...
        const auto location{gl_function{&GladGLContext::GetUniformLocation}(get_index(m_Resource), name.data())};
        if(location == -1)
