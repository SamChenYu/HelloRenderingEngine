...
namespace avocet::opengl {
    ...
    namespace {
        ...
        [[nodiscard]]
        GLint get_max_message_length(const GladGLContext& ctx, [[maybe_unused]] std::source_location loc) {
            GLint maxLen{};
            // Direct call since error checking functions can't use gl_function wrapper
            if(ctx.GetIntegerv) {
                ctx.GetIntegerv(GL_MAX_DEBUG_MESSAGE_LENGTH, &maxLen);
            }
            return maxLen;
        }
        ...
        std::optional<debug_info> get_next_message(const GladGLContext& ctx, std::source_location loc) {
            ...
            // Direct call since error checking functions can't use gl_function wrapper
            GLuint numFound = 0;
            if(ctx.GetDebugMessageLog) {
                numFound = ctx.GetDebugMessageLog(1, to_gl_sizei(message.size()), &source, &type, &id, &severity, &length, message.data());
            }
