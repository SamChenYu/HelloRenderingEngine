...
namespace avocet::opengl {
    ...
    namespace {
        ...
        GLint get_max_message_length(std::source_location loc) {
            GLint maxLen{};
            gl_function{unchecked_debug_output, &GladGLContext::GetIntegerv, loc}(GL_MAX_DEBUG_MESSAGE_LENGTH, &maxLen);
            return maxLen;
        }
        ...
        std::optional<debug_info> get_next_message(std::source_location loc) {
            ...
            const auto numFound{gl_function{unchecked_debug_output, &GladGLContext::GetDebugMessageLog}(1, to_gl_sizei(message.size()), &source, &type, &id, &severity, &length, message.data())};
        ...
        [[nodiscard]]
        error_code get_error() { return error_code{gl_function{unchecked_debug_output, &GladGLContext::GetError}()}; }
