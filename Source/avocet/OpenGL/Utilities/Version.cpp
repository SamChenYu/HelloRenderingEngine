...
namespace avocet::opengl{
    ...
    std::string get_vendor() {
        return {std::bit_cast<const char*>(gl_function{&GladGLContext::GetString}(GL_VENDOR))};
    }

    [[nodiscard]]
    std::string get_renderer() {
        return {std::bit_cast<const char*>(gl_function{&GladGLContext::GetString}(GL_RENDERER))};
    }
